...
class RecoverStage:
    ...
    def _recover(self):
        ...
        if math.isfinite(x) and math.isfinite(y) and 0.2 < x < 7.8 and -7.8 < y < -0.2:
            # Real dynamic objects remember a failed attempt -- a knocked one stays shoved, a toppled one
            # lies on its side -- so a retry would reach for a target that has moved.
            ...
            # base is sane, nothing else to do
            return
        # A latched safety abort skips re-homing: `world.reset()` makes the STATE clean, not the MOTION
        # validated, so every write after it is exactly as unvalidated as before.
        if self._safety_abort:
            print(f">>> RECOVER SKIPPED: base at ({x:.1f},{y:.1f}) is bad, but a checkpoint "
                  f"SAFETY ABORT is latched -- the run halts, so there is no next cycle to "
                  f"un-brick; re-homing would drive the arm on a path nothing validated",
                  flush=True)
            return
        print(f">>> RECOVER: base at ({x:.1f},{y:.1f}) is bad -> world.reset + re-home", flush=True)
        self.grip = None
        # fingers back under kinematic control
        self._finger_cmd = None
        # rebuild the physics view (clears the NaN state)
        self.world.reset()
        ...
        # Clear drive feedforward so stale deltas from before the teleport are not applied.
        self._drv_prev = None
